...
int cercaFoglia(nodo *root, int key, nodo *&t) {
    ...
    int i = -1;
    if (root->right) {
        i = cercaFoglia(root->right, key, t);
    }
    if (i == -1) {
        i = cercaFoglia(root->left, key, t);
    }
    if (i == -1) {
        return -1;
    } else {
...
// POST=(se c'è una foglia con info=x, allora restituisce col return la
// profondità minima di tale foglia e assegna a t il puntatore ad essa,
...
int main() {
    ...
    printLevelOrder(root); // decommentare per vedere la stampa dell'albero
    // livello-per-livello
    nodo *tmp = root;

    cout << "start" << endl;
    int altezza = cercaFoglia(
        root, key, tmp); // CHIAMATA ALLA FUNZIONE RICORSIVA DA IMPLEMENTARE
